...
import { ShoppingCart, Loader2, Trash2, ChevronRight, Search, X } from 'lucide-react';
...
const CartsPanel = ({ 
  ...
  const handleRemoveCart = async (productId) => {
    ...
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/api/carts/${productId}/`, config);
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-gray-100">
          <div className="flex items-center justify-between p-4">
            <div>
              ...
              <p className="text-sm text-gray-500">{carts.length} items saved</p>
            </div>
            <button 
              onClick={onClose}
        ...
        {loading ? (
          ...
          <div className="py-12 px-4 text-center">
            <ShoppingCart className="h-16 w-16 mx-auto mb-4 stroke-1 text-gray-400" />
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {searchTerm ? 'No matching items found' : 'No cart yet'}
            </h3>
            <p className="text-gray-500 mb-6">
              {searchTerm 
                ? 'Try adjusting your search terms'
                : 'Items added to your carts will appear here'
              }
            </p>
            <button
